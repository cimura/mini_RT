...
/*   Updated: 2025/03/29 00:06:44 by ttakino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	int recursion_level)
{
	...
	if (recursion_level > MAX_RECURSIVE_LEVEL)
		return (color);
	// 交点の数を計算
	closest_intersection = find_intersection_minimum_distance(world, trajectory.ray);
	// 交点がない場合は背景色を置いてreturn  余分な計算を省くことができる
	if (closest_intersection.t < 0)
		return (color);
	// 法線ベクトルを計算
	calculate_intersections_normal_vector(&closest_intersection, trajectory.ray);
	// Phong反射モデルに則ってピクセルの色を計算
	color = calculate_phong_radiance(world, closest_intersection, trajectory.ray);
	// もし完全鏡面反射か屈折をするならばそれらの色を足す
	color = dcolor_add(color,
		calculate_catadioptric_radiance(world, closest_intersection, trajectory, recursion_level));
	return (color);
}

//void	debug_printer(t_xy in_screen, t_world world, t_intersection i, t_ray ray)

static t_dcolor ray_trace(t_world world, t_trajectory trajectory)
{
	t_dcolor	color;

	color = ray_trace_recursive(world, trajectory, 0);
	return (color);
}
...
void render_scene(t_world world)
{
	...
	trajectory.ray.coordinates_vec = world.camera.coordinates_vec;
	...
	while (in_screen.y < HEIGHT - 1)
	{
		...
		while (in_screen.x < WIDTH - 1)
		{
			...
			trajectory.ray.orientation_vec = get_rays_orientation_vector(in_screen, world.camera);
			//t_vector screen_vec;
			...
			color = ray_trace(world, trajectory);
