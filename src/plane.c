...
/*   Updated: 2025/04/22 21:46:59 by ttakino          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	const t_object *plane, t_texture *tex)
{
	...
	if (on_map.x < 0)
		on_map.x = 1 - fmod(fabs(on_map.x), width) / (width);
	else
		on_map.x = fmod(on_map.x, width) / (width);
	if (on_map.y < 0)
		on_map.y = 1 - fmod(fabs(on_map.y), height) / (height);
	else
		on_map.y = fmod(on_map.y, height) / (height);
	return (on_map);
}
