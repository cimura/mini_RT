#include "parser.h"
#include "draw.h"

void	print_err_msg(int errnum)
{
	printf("Error\n");
	if (errnum == INV_NUM_OF_ARGS)
		printf("Too many/too few arguments.\n");
	else if (errnum == FILE_TOO_LARGE)
		printf(".rt file is too large.\n");
	else if (errnum == INV_FILENAME)
		printf("The argument file extension must be .rt.\n");
	else if (errnum == FILE_NOT_FOUND)
		printf("File not found.\n");
	else if (errnum == CANT_READ_FILE)
		printf("Cannot read file.\n");
}

size_t  ft_strlen(const char *str)
{
	size_t	i;

	if (!str)
		return (0);
	i = 0;
    while (str[i] != '\0')
		i++;
    return (i);
}

int     ft_strrncmp(const char *s1, const char *s2, size_t size)
{
        int     i;
        int     j;

        if (!s1 || !s2)
                return (0);
        i = ft_strlen(s1) - 1;
        j = ft_strlen(s2) - 1;
        while (--size && i && j && s1[i] == s2[j])
        {
			i--;
			j--;
        }
        return ((unsigned char)s1[i] - (unsigned char)s2[j]);
}

int	check_num_of_args(int argc)
{
	if (argc != 2)
	{
		print_err_msg(INV_NUM_OF_ARGS);
		return (1);
	}
	return (0);
}

int	check_file_name(char *filename)
{
	if (ft_strrncmp(filename, ".rt", 3) != 0)
	{
		print_err_msg(INV_FILENAME);
		return (1);
	}
	return (0);
}

#define BUF_SIZE 1025

int	read_rt_file(char *filename, char *buf)
{
	int	fd;
	int	file_bytes;

	fd = open(filename, O_RDONLY);
	if (fd < 0)
	{
		print_err_msg(FILE_NOT_FOUND);
		return (1);
	}
	file_bytes = read(fd, buf, BUF_SIZE);
	if (file_bytes < 0)
	{
		print_err_msg(CANT_READ_FILE);
		return (1);
	}
	if (file_bytes > BUF_SIZE - 1)
	{
		print_err_msg(FILE_TOO_LARGE);
		return (1);
	}
	buf[file_bytes] = '\0';
	return (0);
}

int	parse_rt_file(char *buf)
{
	// TODO t_sceneに当てはめる
	printf ("%s\n", buf);
	return (0);
}

int	parse_arguments(int argc, char **argv)
{
	char	buf[BUF_SIZE];

	if (check_num_of_args(argc) != 0)
		return (1);
	// filename check
	if (check_file_name(argv[1]) != 0)
		return (1);
	// read file
	if (read_rt_file(argv[1], buf) != 0)
		return (1);
	// parse file
	if (parse_rt_file(buf) != 0)
		return (1);
	// setup scene data from .rt file
	return (0);
}